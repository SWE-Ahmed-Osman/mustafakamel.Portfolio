...
const AboutExperiance = () => {
  ...
  return (
    <section ref={ref} className="experiance_section">
      <Container>
        <ScrollTransition elementTop={elementTop} classes="flex header_experiance">
          <SectionTitle Title={t('experiance')}/>
          <ScrollTransition elementTop={elementTop} delay={.1}>
          <MainBtn To='resume'>{icon} {t('Resume')}</MainBtn>
          </ScrollTransition>
        </ScrollTransition>
        <div className="row">
          {companies.map((comp, i) => <SingleExpericane elementTop={elementTop} index={i} key={comp.id} {...comp} />)}
        </div>
      </Container>
    </section>
  );
